...
void main() => group("Blog", () {
	group(".fromJson()", () {
		...
		test("should return an initialized instance with a non-empty map", () {
			final blog = Blog.fromJson({
				"blog": "https://docs.belin.io/akismet.dart",
				"blog_charset": "UTF-8",
				"blog_lang": "en, fr"
			});
			...
			expect(blog.url, equals(Uri.https("docs.belin.io", "/akismet.dart")));
		});
	});

	group(".toJson()", () {
		test("should return only the blog URL with a newly created instance", () {
			final data = Blog(Uri.https("docs.belin.io", "/akismet.dart")).toJson();
			expect(data, hasLength(1));
			expect(data["blog"], equals("https://docs.belin.io/akismet.dart"));
		});

		test("should return a non-empty map with an initialized instance", () {
			final data = Blog(Uri.https("docs.belin.io", "/akismet.dart"), charset: "UTF-8", languages: ["en", "fr"]).toJson();
