...
void main() => group("Blog", () {
	group(".fromJson()", () {
		...
		test("should return an initialized instance with a non-empty map", () {
			...
			expect(blog.url, equals(Uri.https("cedx.github.io", "/akismet.dart")));
		});
	});

	group(".toJson()", () {
		test("should return only the blog URL with a newly created instance", () {
			final data = Blog(Uri.https("cedx.github.io", "/akismet.dart")).toJson();
		...
		test("should return a non-empty map with an initialized instance", () {
			final data = Blog(Uri.https("cedx.github.io", "/akismet.dart"), charset: "UTF-8", languages: ["en", "fr"]).toJson();
